...
On macOS a window is excluded from screen capture when its `NSWindow.sharingType` is `NSWindowSharingNone` — `WebviewWindow::set_content_protected(true)` sets that, and the capturing compositor then renders whatever is **behind** the window in its region instead of the window's pixels. Verified empirically (v2.1.3): `sharingType = .none` is honored by `screencapture`, and by every ScreenCaptureKit path (display exclude-filter, display include-filter, single-window) that Zoom/Meet/recorders use — a single-window SCK capture of a protected window comes back blank. The one residual is that a `.none` window still appears in `SCShareableContent`'s window *enumeration* (its pixels are excluded, but it is not hidden from the list). So `.none` is both necessary and sufficient for pixel exclusion — the requirement is simply that **every** window carries it, always.

**The invariant (v2.1.3): no window exists unprotected while `screenshareHidden` is on.** Earlier code built each window explicitly *unprotected* (`.content_protected(false)`) and flipped it on afterward with a separate `set_content_protected` call whose `Result` was discarded — so "captured" was the fallback if that call didn't take effect, with nothing to re-check it, and the **settings window was never protected at all**. Now:

- Every window (`prompter`, `settings`) is **built** with `.content_protected(screenshareHidden && !captureAllowed)`, so it is protected from its first frame — the fallback state is *excluded*, not captured.
- `reassert_screenshare(app, context)` re-applies protection to **all** windows and logs the resulting `sharingType` per window. It runs after every lifecycle event that could reconfigure or reset a window: launch, `create_prompter_window` (mode switch), `elevate_notch_window`, `get_notch_metrics`, `set_config`, and on every window `Focused(true)` event.
- `apply_screenshare_mode()` reads back the live `sharingType` after setting it and logs it, so a silent failure is visible rather than assumed away.

**Diagnostic + verification (deliverables 1–2).** `capture_debug` (command; also emitted as the `capture-debug` event and shown by the `?capturedebug=1` prompter overlay and the Settings status line) reports each window's live `sharingType`, its name, `protected` bool, and the `screenshareHidden`/`captureAllowed`/`effective` gates — read at runtime from the real `NSWindow`, never assumed. `scripts/capture-check.mjs` drives the app through three states (pill collapsed, panel expanded, tracking session), asserts every content window is `sharingType == none` via CoreGraphics, and cross-checks by capturing the display with `screencapture` and diffing the window rect against the same state captured unprotected — failing if the app's own pixels appear. (The tray status item is an `NSStatusItem`, not a Tauri window, so it is not content-protectable; it shows only the menu-bar icon, never script content, and the check ignores it.)

**Dev-only escape hatch (this fork):** launching with `TELEPROMPTER_ALLOW_CAPTURE=1` disables protection so the pill can be screenshotted (README captures, visual debugging); `TELEPROMPTER_DEMO_PARAMS="view=read&trackdemo=1"` navigates the prompter to the demo-hook URL the snapshot suite uses, so fixed UI states can be driven in the real app. Neither is set in normal operation, so protection stays on by default. `scripts/readme-shots-real.sh` automates that flow for `docs/screenshots/`. Default is ON (`Config::default`).
